<template>
    <div class="tag-view-container">
        <el-tag v-if="visitedTagViews.length===1 && visitedTagViews[0].path==='/dashboard'" :class="$route.path===visitedTagViews[0].path?'active':''" type="success" size="small" disable-transitions>
            {{$t(visitedTagViews[0].meta.title)||''}} {{visitedTagViews[0].params.id?`【${visitedTagViews[0].params.id}】`:''}}
        </el-tag>
        <template v-else>
            <el-tag v-for="(item, index) in visitedTagViews" :key="item.path" :class="$route.path===item.path?'active':''" type="primary" size="small" closable disable-transitions @click="handleClick(item)" @close="handleClose(index, $route.path===item.path)">
...
export default {
  ...
  methods: {
    ...
    handleClick (item) {
      let lastIndex = this.visitedTagViews.findIndex(i => i.name === this.$route.name)
      let newIndex = this.visitedTagViews.findIndex(i => i.name === item.name)
